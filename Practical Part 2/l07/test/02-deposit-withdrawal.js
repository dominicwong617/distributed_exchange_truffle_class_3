...
contract('Exchange Basic Tests', function (accounts) {
    ...
    //        return myExchangeInstance.depositEther({from: accounts[0], value: web3.utils.toWei(1, "ether")});
    ...
    //        assert.equal(balanceInWei.toNumber(), web3.utils.toWei(1, "ether"), "There is one ether available");
    //        assert.equal(web3.utils.toWei(1, "ether"), balanceBeforeTransaction.toNumber() - balanceAfterDeposit.toNumber() - gasUsed, "Balances of account are the same");
    ...
    it("should be possible to Deposit and Withdrawal Ether", function () {
        ...
        return exchange.deployed().then(function (instance) {
            ...
            transaction = await web3.eth.getTransaction(txHash.receipt.transactionHash);
            gasUsed += txHash.receipt.cumulativeGasUsed * transaction.gasPrice; //here we have a problem
            balanceAfterDeposit = web3.eth.getBalance(accounts[0]);
            return myExchangeInstance.getEthBalanceInWei.call();
        }).then(async function (balanceInWei) {
            assert.equal(balanceInWei, web3.utils.toWei("1", "ether"), "There is one ether available");
            balanceBeforeTransaction = await balanceBeforeTransaction;
            balanceAfterDeposit = await balanceAfterDeposit;
            assert.isAtLeast(balanceBeforeTransaction - balanceAfterDeposit, new Number(web3.utils.toWei("1", "ether")),  "Balances of account are the same");
            return myExchangeInstance.withdrawEther(web3.utils.toWei("1", "ether"));
        }).then(function (txHash) {
            balanceAfterWithdrawal = web3.eth.getBalance(accounts[0]);
            return myExchangeInstance.getEthBalanceInWei.call();
        }).then(async function (balanceInWei) {
            balanceAfterWithdrawal = await balanceAfterWithdrawal;
            balanceBeforeTransaction = await balanceBeforeTransaction;
            // Should use BN.js
            assert.equal(balanceInWei, 0, "There is no ether available anymore");
            assert.isAtLeast(new Number(balanceAfterWithdrawal), new Number(balanceBeforeTransaction) - gasUsed*2, "There is one ether available");
        });
    });
